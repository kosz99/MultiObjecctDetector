...
class GIOULoss(nn.Module):
    '''
    GIOU Loss for regression bounding box compatible with FCOS fashion bounding box regression (l*, r*, t*, b*) - https://giou.stanford.edu/
    '''
    def __init__(self):
        super().__init__()
    ...
    def forward(self, input, label):
        ...
        '''
        result = self.giou(input, label)


        return (1 - result).unsqueeze(1)
    
    def giou(self, input, label):
        '''
        Computing GIOU metric
        ...
        convex_hall = (x2_convex + abs(x1_convex))*((y2_convex) + abs(y1_convex))

        IOU = intersection/(union+1e-8)

        return IOU - ((convex_hall - union)/(convex_hall+1e-8))
